...
/*   Updated: 2023/01/22 18:09:51 by achane-l         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	transplant(Node **root, Node *node, Node *new_child){
	Node *parent = node->parent;

	if (parent && parent->left_child == node)
		parent->left_child = new_child;
	else if (parent && parent->right_child == node)
...
Node*	minimum_of_tree(Node *root){
	while (root && root->left_child)
		root = root->left_child;
	return (root);
}

Node*	maximum_of_tree(Node *root){
...
void	deleteFix(Node **root,Node *fixer){
	// Node *s = NULL;

	while (*root != fixer && (!fixer || fixer->color == BLACK)){
		// s = fixer->parent->right_child;
		// if (s && s->color == RED){
		// 	s->color = BLACK;
		// 	fixer->parent->color = RED;
		// 	left_rotation(root, fixer->parent);
		// 	s = fixer->parent->right_child;
		// }
		// if ((!s->left_child || (s->left_child && s->left_child->color == BLACK)) && (!s->right_child || (s->right_child && s->right_child->color == BLACK))){
		// 	s->color = RED;
		// 	fixer = fixer->parent;
		// }
		// else if ((!s->right_child || (s->right_child && s->right_child->color == BLACK)){
...
void	delete_node(Node **root, Node *node){
	Node *min = node;
	bool org_color = min->color;
	Node *fixer =NULL;
	
	if (node->left_child == NULL){
		fixer = node->right_child;
		transplant(root, node, fixer);
	}
	else if (node->right_child == NULL){
		fixer = node->left_child;
		transplant(root, node, fixer);
	}
	else{
		min = minimum_of_tree(node->right_child);
		org_color = min->color;
		fixer = min->right_child;
		if (min->parent == node)// voir
			fixer->parent = min;
		else {
			transplant(root, min, min->right_child);
